...
class Todo {
  ...
  // Desarrollado por TODO Mining
  /**
   *
   * @var string
   */
  private $framework    = 'TODO Framework'; // Ahora este solo será el nombre idenficador del framework y no el nombre del sistema como tal
  private $version      = '1.0.0';         // versión actual del framework y no del sistema en desarrollo, la versión del sistema deberá ser actualizada directamente en todo_config.php
  private $lng          = 'es';
  private $uri          = [];
  ...
  private function init_load_functions() {
    ...
    $file = 'todo_custom_functions.php';
    if(!is_file(FUNCTIONS.$file)) {
      die(sprintf('El archivo %s no se encuentra, es requerido para que %s funcione.', $file, $this->framework));
    }
  ...
  private function init_globals() {
    // Objeto TODO que será insertado en el footer como script javascript dinámico para fácil acceso
    todo_obj_default_config();
